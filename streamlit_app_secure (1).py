...
class ConstructionItemClassifier:
    ...
    def classify(self, name, work_category='', parent_category=''):
        ...
        # 信頼度を初期化
        confidence = "高"  # デフォルトは高信頼度
        
        # 親カテゴリが設備系の場合
        if '設備工事' in parent_normalized:
            if any(k in parent_normalized for k in ['電気', '電力', '電灯', '照明', '電話', 'インターネット', 'テレビ', 'インターホン', 'ITV', '）ＴＶ', '火災報知', '避雷']):
                return '電気設備', "高"
            elif any(k in parent_normalized for k in ['給排水', '給水', '給湯', '排水', '衛生器具', 'ガス', '消火', '空調']):
                return '空気調和設備', "高"
        
        # 判定優先順位
        if self.is_electric_equipment(name): return '電気設備', "高"
        if self.is_hvac_equipment(name): return '空気調和設備', "高"
        if self.is_roof(name): return '4.1 屋根', "高"
        if self.is_pile_foundation(name): return '2.2 杭・基礎', "高"
        if '杭工事' in str(work_category) and '施工費' in normalized:
            return '2.2 杭・基礎', "高"
        if self.is_concrete(name): return '3.1 コンクリート', "高"
        if self.is_steel_frame(name): return '3.3 鉄骨', "高"
        if self.is_rebar(name): return '3.4 鉄筋', "高"
        if self.is_other_structure(name): return '3.9 その他', "高"
        if self.is_exterior_wall(name): return '4.2 外壁', "高"
        if self.is_exterior_opening(name): return '4.3 外部開口部', "高"
        if self.is_interior_floor(name): return '5.1 内部床', "中"  # 床は判定が難しい場合がある
        if self.is_interior_wall(name): return '5.2 内壁', "中"  # 壁も判定が難しい場合がある
        if self.is_interior_opening(name): return '5.3 内部開口部', "高"
        if self.is_ceiling(name): return '5.4 天井', "高"
        if self.is_interior_misc(name): return '5.9 内部雑', "高"
        if self.is_excluded(name): return '0.0 対象外', "高"
    ...
    try:
        ...
        for i in range(data_start_row, max_row + 1):
            ...
            if name and str(name).strip() != '':
                classification, confidence = classifier.classify(name, work_category, current_parent)
                ...
                if classification:
                    ...
                    # 信頼度が中または低の場合、要確認リストに追加
                    if confidence in ["中", "低"]:
                        uncertain_items.append({
                            "行番号": excel_row,
                            "名称": str(name)[:50],  # 最大50文字
                            "判定結果": classification,
                            "信頼度": confidence
                        })
        ...
        return output, stats, uncertain_items
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 判定を実行", type="primary"):
            ...
            # 処理実行
            output, stats, uncertain_items = process_excel_streamlit(uploaded_file)
            ...
            if output and stats:
                ...
                with metric_cols[2]:
                    st.metric("要確認", f"{len(uncertain_items):,}", 
                             delta=f"{len(uncertain_items)/total_items*100:.1f}%" if total_items > 0 else "0%",
                             delta_color="inverse")
                with metric_cols[3]:
                ...
                # 要確認項目の表示（信頼度が中・低のもの）
                if uncertain_items:
                    st.subheader("⚠️ 要確認項目")
                    
                    st.warning(f"""
                    以下の **{len(uncertain_items)}件** は判定の信頼度が低いため、確認をおすすめします。  
                    これらの項目は手動で確認・修正してください。
                    """)
                    
                    # タブで信頼度別に分類
                    tab1, tab2 = st.tabs(["🟡 中信頼度", "🔴 低信頼度"])
                    
                    with tab1:
                        medium_items = [item for item in uncertain_items if item["信頼度"] == "中"]
                        if medium_items:
                            st.info(f"**{len(medium_items)}件** - 判定結果はおおむね正しいと思われますが、確認を推奨します")
                            medium_df = pd.DataFrame(medium_items)
                            st.dataframe(
                                medium_df,
                                use_container_width=True,
                                column_config={
                                    "行番号": st.column_config.NumberColumn("行番号", help="Excelの行番号"),
                                    "名称": st.column_config.TextColumn("名称", width="large"),
                                    "判定結果": st.column_config.TextColumn("判定結果", width="medium"),
                                    "信頼度": st.column_config.TextColumn("信頼度", width="small")
                                }
                            )
                        else:
                            st.success("✅ 中信頼度の項目はありません")
                    
                    with tab2:
                        low_items = [item for item in uncertain_items if item["信頼度"] == "低"]
                        if low_items:
                            st.error(f"**{len(low_items)}件** - 判定結果が不確実です。必ず確認してください")
                            ...
                            st.success("✅ 低信頼度の項目はありません")
                    
                    # CSVダウンロードボタン
                    st.markdown("---")
                    uncertain_df = pd.DataFrame(uncertain_items)
                    csv = uncertain_df.to_csv(index=False, encoding='utf-8-sig')
                    st.download_button(
                        label="📋 要確認リストをCSVでダウンロード",
                        data=csv,
                        file_name=f"要確認リスト_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv",
                    )
                else:
                    st.success("✅ すべての項目が高信頼度で判定されました！")
